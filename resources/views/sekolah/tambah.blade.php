<x-layout>
    ...
    <div class="w-3/4 border border-slate-200 rounded-xl mx-auto mt-4 shadow-lg p-5">
        <form action="{{ route('sekolah.submit') }}" method="POST">
            @csrf
            ...
            <label for="email" class="block mb-4">
                ...
                <input type="email" id="email" name="email" placeholder="masukkan email..." class="h-12 px-3 py-2 border shadow rounded w-full block text-sm placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500">
            </label>
