<x-layout>
    ...
    <div class="w-3/4 border border-slate-200 rounded-xl mx-auto mt-4 shadow-lg  p-5">
        <form action="{{ route('user.update', $user->id) }}" method="POST">
            ...
            <label for="name" class="block mb-4">
                ...
                <input type="text" id="name" name="name" placeholder="masukkan nama..." value="{{ $user->name }}"  class="h-12 px-3 py-2 border shadow rounded w-full block text-sm placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500">
            </label>
            @error('name')
            ...
            <label for="email" class="block mb-4">
                ...
                <input type="text" id="email" name="email" placeholder="masukkan email..." value="{{ $user->email }}" class="h-12 px-3 py-2 border shadow rounded w-full block text-sm placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500">
            </label>
            @error('email')
    ...
    <div class="w-3/4 border border-slate-200 rounded-xl mx-auto mt-8 shadow-lg  p-5">
